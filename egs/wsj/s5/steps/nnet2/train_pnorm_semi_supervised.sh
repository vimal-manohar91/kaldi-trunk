...
# Check some files.
for f in $data_sup/feats.scp $lang/L.fst $alidir/ali.1.gz $alidir/final.mdl $alidir/tree; do
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  ...
  steps/nnet2/get_egs_semi_supervised.sh $spk_vecs_opt --samples-per-iter $samples_per_iter --num-jobs-nnet $num_jobs_nnet \
