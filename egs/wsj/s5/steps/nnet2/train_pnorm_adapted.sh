...
splice_width=7 # meaning +- 4 frames on each side for second LDA
...
if [ $stage -le -5 ]; then
  ...
  steps/nnet2/get_lda.sh $lda_opts --splice-width $splice_width --cmd "$cmd" --no-fmllr $no_fmllr  $data $lang $alidir $dir || exit 1;
fi
...
if [ $stage -le -4 ] && [ -z "$egs_dir" ]; then
  ...
  steps/nnet2/get_egs.sh $spk_vecs_opt --samples-per-iter $samples_per_iter --num-jobs-nnet $num_jobs_nnet \
      --splice-width $splice_width --stage $get_egs_stage --cmd "$cmd" $egs_opts --io-opts "$io_opts" --no-fmllr $no_fmllr \
...
if [ $stage -le -3 ] && [ ! -f $egs_dir/relabel.done ]; then
  echo "$0: relabeling examples"
  iters_per_epoch=`cat $egs_dir/iters_per_epoch`  || exit 1;
  x=0;
  while [ $x -lt $iters_per_epoch ]; do
      nnets_ensemble=
      for i in `seq 1 $ensemble_size`; do
        nnets_ensemble="$nnets_ensemble $oldmdl_dir/final.$i.mdl"
      done
      $cmd $parallel_opts JOB=1:$num_jobs_nnet $dir/log/relabel.$x.JOB.log \
        nnet-relabel-egs-ensemble --minibatch-size=2048 $nnets_ensemble ark:$base_egs_dir/egs.JOB.$x.ark ark:$egs_dir/egs.JOB.$x.ark ark:$egs_dir/egs.relabeled.JOB.$x.ark || exit 1;
    x=$[$x+1]
  done
  touch $egs_dir/relabel.done
fi
...
while [ $x -lt $num_iters ]; do
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    $cmd $parallel_opts JOB=1:$num_jobs_nnet $dir/log/train.$x.JOB.log \
      ...
      ark:$egs_dir/egs.relabeled.JOB.$[$x%$iters_per_epoch].ark ark:- \| \
