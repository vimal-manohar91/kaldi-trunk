...
if [ $# != 6 ]; then
  ...
  echo "                                                   # the middle."
  echo "  --weight-threshold <0.7>                         # Threshold on the confidence factor below which the "
...
cp $latdir/../tree $dir   # ali.*.gz must be converted to this tree if needed
# TODO (Vimal 22-Jan-14): The tree in $alidir and $latdir/.. must be the same. Otherwise the pdf ids would be meaningless.
# Possibly convert alignments from one tree to another

# Get list of validation utterances. 
# TODO (Vimal 22-Jan-14): Does the validation utterances be from the unsupervised dataset as well?
...
if [ -z $feat_type ]; then
  ...
  #if [ -f $latdir/../final.mat ] && [ ! -f $transform_dir_unsup/raw_trans.1 ]; then feat_type_unsup=lda; else feat_type_unsup=raw; fi
fi

#if [ "$feat_type" -ne "$feat_type_unsup" ]; then
#  echo "ERROR: Features mismatch for supervised and unsupervised data!"
#  echo "Variance normalization $feat_type for supervised data vs $feat_type_unsup for unsupervised data"
#  exit 1
#fi
...
if [ $stage -le 3 ]; then
  ...
  $cmd $io_opts JOB=1:$nj_unsup $dir/log/get_weighted_egs.JOB.log \
    ...
    "ark,s,cs:gunzip -c $latdir/best_path_ali.JOB.gz | ali-to-pdf $latdir/../final.mdl ark:- ark:- | ali-to-post ark:- ark:- |" \
