...
$max_segment_length = 10.0; # Maximum segment length while we are merging segments...
                            # it will not allow merging segments to make segments longer than this.
$hard_max_segment_length = 10.0; # A hard maximum on the segment length; it will
...
GetOptions('silence-proportion:f' => \$silence_proportion,
           ...
           'min-inter-utt-silence-length:f' => \$min_inter_utt_silence_length,
           'split-on-noise-transitions:s' => \$split_on_noise_transitions,
...
if (@ARGV != 0) {
  print STDERR "$0:\n" .
               ...
               "--min-inter-utt-silence-length <float> (default: $min_inter_utt_silence_length), affects silence length between seprate utterances\n" .
               "--split-on-noise-transitions <true|false> (default: $split_on_noise_transitions)\n" .
...
($split_on_noise_transitions eq 'false' ||
 $split_on_noise_transitions eq 'true') || 
  die "Option --split-on-noise-transitions takes args true or false";
...
sub set_silence_proportion {
  ...
  # Keep a left and right target frames so that the minimum utterance length 
  ...
  while ($num_segment_frames < $target_segment_frames || $left_segment_frames < $left_target_frames || $right_segment_frames < $right_target_frames) {
    ...
    for (my $i = 0; $i < @active_frames; $i++) {
      ...
      if ($num_segment_frames >= $target_segment_frames && $left_segment_frames >= $left_target_frames && $right_segment_frames >= $right_target_frames) {
        last;
      }
    }
        ...
        if ($split_on_noise_transitions eq 'true') {
          ...
          for ($k = $n; $k < $p; $k++) {
            if ($A[$k] == 1) {
              ...
              if ($k > $n && $A[$k-1] == 1) {
                # But previous frame is noise
                $NE[$k-$n] = 1;
              }
            }
          }
          # Handle special case where the last frame is noise
          if ($A[$p-1] == 1) {
            $NE[$p-$n] = 1;
          }
        
          # store the length of noise from the beginning of 
          # noise in the hash table noise_length which is 
          # indexed by the noise starting boundary
          my @noise_boundaries = ();
          my @noise_length = ();
          ...
          # Sort on decreasing length of silence segment,
          # so we first splits long silence segments
          my @sorted_noise_boundaries = sort { $noise_length[$b] <=> $noise_length[$a] } @noise_boundaries;
