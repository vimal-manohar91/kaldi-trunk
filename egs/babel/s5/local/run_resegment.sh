...
if [ ! -f exp/tri4_ali_sub3/.done ]; then
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
    data/train_sub3 data/lang exp/tri4 exp/tri4_ali_sub3 || exit 1;
  touch exp/tri4_ali_sub3/.done
fi

if [ ! -f exp/tri4b_seg/.done ]; then
  steps/train_lda_mllt.sh --cmd "$train_cmd" --realign-iters "" \
    1000 10000 data/train_sub3 data/lang exp/tri4_ali_sub3 exp/tri4b_seg || exit 1;
  touch exp/tri4b_seg/.done
fi

if [ ! -f exp/tri4_ali_all/.done ]; then
  steps/align_fmllr.sh --nj $train_nj --cmd "$train_cmd" \
    data/train data/lang exp/tri4 exp/tri4_ali_all || exit 1;
  touch exp/tri4_ali_all/.done
fi

if [ ! -f exp/tri4b_seg/graph.done ]; then
  # Make the phone decoding-graph.
  steps/make_phone_graph.sh data/lang exp/tri4_ali_all exp/tri4b_seg || exit 1;
  touch exp/tri4b_seg/graph.done
fi
...
for data in train dev10h dev2h eval; do
  ...
  if [ ! -f exp/tri4b_seg/decode_${data}_orig/.done ]; then
    steps/decode_nolats.sh --write-words false --write-alignments true \
      --cmd "$decode_cmd" --nj $my_nj --beam 7.0 --max-active 1000 \
      exp/tri4b_seg/phone_graph data_reseg/${train}_orig exp/tri4b_seg/decode_${data}_orig || exit 1
    touch exp/tri4b_seg/decode_${data}_orig/.done
  fi
  
  if [ ! -f exp/tri4b_resegment_${data}/.done ]; then
    steps/resegment_data.sh --cmd "$train_cmd" data_reseg/${data}_orig data/lang \
      exp/tri4b_seg/decode_${data}_orig data_reseg/$data exp/tri4b_resegment_${data}
    touch exp/tri4b_resegment_${data}/.done
  fi
  ...
  if [ "$data" == "train" ]; then
    ...
    if [ ! -f exp/tri4_ali_train/.done ]; then
      steps/align_fmllr.sh --nj $my_nj --cmd "$train_cmd" \
        data/train data/lang exp/tri4 exp/tri4_ali_train || exit 1;
      touch exp/tri4_ali_train/.done
    fi
    ...
    if [ ! -f exp/data_reseg/train/text ]; then
      ...
      steps/resegment_text.sh --cmd "$train_cmd" data/train data/lang \
        exp/tri4_ali_train data_reseg/train exp/tri4b_resegment_train
    fi
  fi
  ...
  if [ ! -f data_reseg/${data}/.done ]; then
    ...
    steps/make_plp --cmd "$train_cmd" --nj $my_nj data_reseg/${data} exp/make_plp/${data} $plpdir 
    # caution: the new speakers don't correspond to the old ones, since they now have "sw0" at the start..
