...
if [ ! -f $data_out/.done ]; then
  utils/extract_insertions.py $extract_insertions_opts $decode_ctm.sgml > $data_out/insertions.txt 2> exp/tri4b_augment_train/extract_insertions.log || exit 1
  
  utils/add_fillers_to_transcription.py $add_fillers_opts $human_ctm $decode_ctm $data_out/insertions.txt $human_segments_file > $data_out/ctm.augmented 2> exp/tri4b_augment_train/add_fillers.log || exit 1
