...
if [ $# != 6 ]; then
  ...
  echo "  --ext-alidir                                     # Extra alignment dir, typically from a better 
                                                           # model like sgmm5_ali. Used as alignments for 
                                                           # training the VAD GMMs after initialization."
  exit 1;
fi
...
if [ $stage -le -3 ]; then
  $cmd JOB=1:$nj $dir/log/acc.JOB.log \
    ...
    gmm-acc-stats $alidir/final.mdl "$feats" ark:- $dir/JOB.acc || exit 1
  
  [ `ls $dir/*.acc | wc -w` -ne "$nj" ] && echo "$0: Wrong #accs: $(ls $dir/*.acc | wc -w) vs $nj" && exit 1;

  echo "$0: clustering model $alidir/final.mdl to get initial speech GMMs"
  $cmd $dir/log/cluster_speech.log \
    merge-gaussians-to-gmm --binary=false \
    --intermediate-num-gauss=$((speech_num_gauss*4)) \
    --gmm-num-gauss=$speech_num_gauss \
  ...
  $cmd $dir/log/cluster_silence.log \
    merge-gaussians-to-gmm --binary=false \
    --intermediate-num-gauss=$((silence_num_gauss*4)) \
    --gmm-num-gauss=$silence_num_gauss \
...
} | sort -k 1 -n > $dir/data/phone_map.txt

if [ $stage -le -1 ]; then
  $cmd JOB=1:$nj $dir/log/get_text.JOB.log ali-to-phones --per-frame=true \
    $alidir/final.mdl "ark:gunzip -c $alidir/ali.JOB.gz|" ark,t:- \| \
    utils/apply_map.pl -f 2- $dir/data/phone_map.txt '>' $dir/data/text.JOB
  for n in `seq 1 $nj`; do cat $dir/data/text.$n; done | sort > $dir/data/text || exit 1
  rm $dir/data/text.*
...
echo "$0: Using alignments from $alidir"
...
if [ $stage -le 0 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/convert_align.JOB.log \
    ali-to-phones --per-frame=true $alidir/final.mdl "ark:gunzip -c $alidir/ali.JOB.gz |" ark,t:- \| \
    utils/apply_map.pl -f 2- $dir/data/phone_map.txt \| \
    copy-int-vector ark,t:- "ark:| gzip -c > $dir/ali.JOB.gz" || exit 1
fi
