...
for d in train dev10h dev2h eval; do
  ...
  if [ ! -f $data_out/${dirid}.orig/.done ]; then
    ...
    cp -rT $data_in/${d}_whole $data_out/${dirid}.orig; rm -r $data_out/${dirid}.orig/split*
  ...
  if [ ! -f $data_out/$d.seg/segments ]; then
    t1=$(date +%s)
    ...
    [ ! -f $data_out/$d.seg/segments ] && exit 1
    cp $data_out/$d.seg/segments exp/${tri4b}_whole_resegment_${d}
    t2=$(date +%s)
    total_time=$((total_time + t2 - t1))
  ...
  if [ ! -f $data_out/${dirid}/.done ]; then
    ...
    [ -f ${data_out}/$type/segments ] && utils/evaluate_segmentation.pl ${data_out}/$type/segments ${data_out}/$dirid/segments &> exp/${tri4b}_whole_resegment_$d/segment_evaluation.log
