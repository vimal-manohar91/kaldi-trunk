...
cp $decode_dir/../final.mdl $dir
cp $decode_dir/../tree $dir

mkdir -p $out_decode

for i in `seq 0 $[num_sys-1]`; do
  if [ $i -eq 0 ]; then
    file_list="\"ark,s,cs:gunzip -c $out_decode/temp_best_path_ali.$i.JOB.gz | ali-to-pdf $dir/final.mdl ark:- ark:- |\" \"ark,s,cs:gunzip -c $out_decode/temp_weights.$i.JOB.gz |\""
  else
    file_list="$file_list \"ark,s,cs:gunzip -c $out_decode/temp_best_path_ali.$i.JOB.gz | ali-to-pdf $dir/final.mdl ark:- ark:- |\" \"ark,s,cs:gunzip -c $out_decode/temp_weights.$i.JOB.gz |\""
  fi
done
...
for i in `seq 0 $[num_sys-1]`; do
  if [ $stage -lt $i ]; then
    ...
    if [ $i -eq 0 ]; then
      ...
      $cmd JOB=1:$nj $out_decode/log/best_path.$i.JOB.log \
        lattice-best-path --acoustic-scale=0.1 \
        "ark,s,cs:gunzip -c $decode_dir/lat.JOB.gz |" \
        ark:/dev/null "ark:| gzip -c > $out_decode/temp_best_path_ali.$i.JOB.gz" || exit 1
    else
      ...
      $cmd JOB=1:$nj $out_decode/log/best_path.$i.JOB.log \
        lattice-best-path --acoustic-scale=0.1 \
        "ark,s,cs:gunzip -c $decode_dir/lat.JOB.gz |" \
        ark:/dev/null "ark:| convert-ali $model $dir/final.mdl $dir/tree ark:- ark:- | gzip -c > $out_decode/temp_best_path_ali.$i.JOB.gz" || exit 1
      file_list="$file_list \"ark,s,cs:gunzip -c $out_decode/temp_best_path_ali.$i.JOB.gz | ali-to-pdf $dir/final.mdl ark:- ark:- |\" \"ark,s,cs:gunzip -c $out_decode/temp_weights.$i.JOB.gz |\""
    fi
    ...
    $cmd JOB=1:$nj $dir/log/get_post.$i.JOB.log \
      ...
      get-post-on-ali ark,s,cs:- "ark,s,cs:gunzip -c $out_decode/temp_best_path_ali.$i.JOB.gz | convert-ali $dir/final.mdl $model $tree ark,s,cs:- ark:- | ali-to-pdf $model ark,s,cs:- ark:- |" "ark:| gzip -c > $out_decode/temp_weights.$i.JOB.gz" || exit 1
  fi
done

if [ $stage -lt $num_sys ]; then
  for i in `seq 0 $[num_sys-1]`; do 
    $cmd JOB=1:$nj $dir/log/recalibrate_conf.$i.JOB.log recalibrate-conf \
      --primary-system=$i $file_list "ark:| gzip -c > $out_decode/weights.$i.JOB.gz" || exit 1
  done
fi 

if [ $stage -lt "$[num_sys+1]" ]; then
  $cmd JOB=1:$nj $dir/log/combine_conf.JOB.log combine-conf \
    $file_list "ark:| gzip -c > $out_decode/best_path_pdf_ali.JOB.gz" "ark:| gzip -c > $out_decode/weights.JOB.gz" || exit
fi 
