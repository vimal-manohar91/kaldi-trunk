...
def map_prediction(A1, V, phone_map, f = None):
  B = []
  prev_x = None
  len_x = 0
  i = 0
  for x in A1:
    if phone_map[x] == "0":
      B.append("0")
    elif (V != None and i < len(V) and V[i] == "1") or phone_map[x] == "1":
      B.append("4")
    elif phone_map[x] == "2":
      B.append("8")
      # End if
...
def main():
  ...
  parser.add_argument('predictions_file', \
      help='Input predictions as an archive indexed by the recording id')
  parser.add_argument('vad_predictions_dir', \
      help='Directory containing VAD predictions')
  ...
  parser.usage=':'.join(parser.format_usage().split(':')[1:]) \
      + 'e.g. :  cat exp/tri4b_resegment/pred/*.pred | %(prog)s - exp/tri4b_resegment_dev10h/phone_map.txt - > data/dev10h.seg/segments'
  ...
  if options.predictions_file == '-':
    predictions_file_handle = sys.stdin
  else:
    try:
      predictions_file_handle = open(options.predictions_file)
    except IOError as e:
      sys.stderr.write("%s: %s: Unable to open file %s\n" % (sys.argv[0], e, options.predictions_file))
      sys.exit(1)

  temp_dir = ' '.join(options.phone_map.split('/')[:-1]) + "/rttm_classes"
  ...
  for line in predictions_file_handle.readlines():
    splits = line.split()
    f = splits[0]
    A = splits[1:]

    try:
      vad_handle = open("%s/%s.pred" % (options.vad_predictions_dir, f))
    except IOError as e:
    ...
    if vad_handle != None:
      try:
        V = vad_handle.readline().split()[1:]
      except (ValueError, IndexError) as e:
        sys.stderr.write("%s: %s\n" % (sys.argv[0], e))
        sys.exit(1)
    B = map_prediction(A, V, phone_map, f)
    ...
    r = Resegmenter(A, B, f, options, phone_map, stats, reference)
    r.resegment()
