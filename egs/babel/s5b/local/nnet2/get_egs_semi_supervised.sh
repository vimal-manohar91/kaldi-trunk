...
transform_dir_unsup=   # If supplied, overrides latdir
ignore_unsupervised_fmllr_transforms=false  # If the unsupervised FMLLR estimation
                                      # is not very reliable, it would be better
...
if [ $# != 6 ]; then
  ...
  echo "  --ignore-unsupervised-fmllr-transforms <true|false> # Set this to true if the FMLLR estimation for "
  echo "                                                      # unsupervised data is not very reliable (default: false)"
...
# Check some files.
for f in $data_sup/feats.scp $data_unsup/feats.scp $lang/L.fst $alidir/ali.1.gz $alidir/final.mdl $alidir/tree $latdir/weights.1.gz $latdir/../final.mdl $latdir/../tree; do
  [ ! -f $f ] && echo "$0: no such file $f" && exit 1;
done
! $use_pdf_ali && [ ! -f $latdir/best_path_ali.1.gz ] && echo "$0: no such file $latdir/best_path_ali.1.gz" && exit 1
$use_pdf_ali && [ ! -f $latdir/best_path_pdf_ali.1.gz ] && echo "$0: no such file $latdir/best_path_pdf_ali.1.gz" && exit 1
...
if ! $ignore_unsupervised_fmllr_transforms; then
  if [ -f $transform_dir_unsup/trans.1 ] && [ $feat_type != "raw" ]; then
    echo "$0: using transforms from $transform_dir_unsup for unsupervised data"
  ...
  if [ -f $transform_dir_unsup/raw_trans.1 ] && [ $feat_type == "raw" ]; then
    echo "$0: using raw-fMLLR transforms from $transform_dir_unsup for unsupervised data"
...
if [ $stage -le 3 ]; then
  ...
  if $use_pdf_ali; then
    diff $dir/tree $latdir/../tree 2> /dev/null
    [ $? -ne 0 ] && echo "$0: Error: $dir/tree and $latdir/../tree differs" && exit 1
    ...
    $cmd $io_opts JOB=1:$nj_unsup $dir/log/get_weighted_egs.JOB.log \
      ...
      "ark,s,cs:gunzip -c $latdir/best_path_pdf_ali.JOB.gz | ali-to-post ark:- ark:- |" \
