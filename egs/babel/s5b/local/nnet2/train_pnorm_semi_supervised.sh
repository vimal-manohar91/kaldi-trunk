...
if [ $# != 6 ]; then
  echo "Usage: $0 [opts] <supervised-data-dir> <unsupervised-data-dir> <lang> <ali-dir> <lat-dir> <exp-dir>"
  echo " e.g.: $0 data/train data/train_unt data/lang exp/tri3_ali exp/tri3/decode_train_unt.seg exp/tri4_nnet"
  echo ""
  ...
  echo "  --weight-threshold  <float>                      # Frame confidence threshold for frame selection"
...
# Check some files.
for f in $data_sup/feats.scp $lang/L.fst $alidir/ali.1.gz $alidir/final.mdl $alidir/tree $latdir/../final.mdl; do
...
sdata=$data_sup/split$nj
utils/split_data.sh $data_sup $nj
...
if [ $stage -le -4 ]; then
  ...
  steps/nnet2/get_lda.sh $lda_opts --splice-width $splice_width --cmd "$cmd" $data_sup $lang $alidir $dir || exit 1;
fi
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  ...
  local/nnet2/get_egs_semi_supervised.sh $spk_vecs_opt --samples-per-iter $samples_per_iter --num-jobs-nnet $num_jobs_nnet \
    --splice-width $splice_width --stage $get_egs_stage --cmd "$cmd" $egs_opts --io-opts "$io_opts" "${semi_supervised_opts[@]}" \
    --transform-dir-sup $transform_dir_sup --transform-dir-unsup $transform_dir_unsup \
    --weight-threshold $weight_threshold \
    $data_sup $data_unsup $lang $alidir $latdir $dir || exit 1;
fi
