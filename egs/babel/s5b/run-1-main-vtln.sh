...
if [ ! -f data/train/.plp.done ]; then
  steps/make_plp.sh --cmd "$train_cmd" --nj $train_nj data/train exp/make_plp/train plp_novtln
  utils/fix_data_dir.sh data/train
  steps/compute_cmvn_stats.sh data/train exp/make_plp/train plp_novtln
  utils/fix_data_dir.sh data/train
...
if [ ! -f data/train/.vtln.done ]; then
  if [ ! -f exp/tri3c/.done ]; then
    echo ---------------------------------------------------------------------
    echo "Starting VTLN training"
    echo ---------------------------------------------------------------------

    steps/train_lvtln.sh --base-feat-type plp --cmd "$train_cmd" --boost-silence $boost_sil \
      $numLeavesTri3 $numGaussTri3 data/train data/lang exp/tri3 exp/tri3c || exit 1 
    touch exp/tri3c/.done
  fi

  mkdir -p data/train_novtln
  cp -rT data/train data/train_novtln
  cp exp/tri3c/final.warp data/train/spk2warp

  if $use_pitch; then
    cp -rT data/train data/train_plp; cp -rT data/train data/train_pitch
    steps/make_plp.sh --cmd "$train_cmd" --nj $train_nj data/train_plp exp/make_plp/train plp_tmp_train || exit 1
    steps/make_pitch_kaldi.sh --cmd "$train_cmd" --nj $train_nj data/train_pitch exp/make_pitch/train pitch_tmp_train || exit 1 
    steps/append_feats.sh --cmd "$train_cmd" --nj $train_nj data/train{_plp,_pitch,} exp/make_pitch/append_train plp || exit 1
    rm -rf {plp,pitch}_tmp_train data/train_{plp,pitch}
  else
    steps/make_plp.sh --cmd "$train_cmd" --nj $train_nj data/train exp/make_plp/train plp || exit 1
  fi

  utils/fix_data_dir.sh data/train
  steps/compute_cmvn_stats.sh data/train exp/make_plp/train_vtln plp
  utils/fix_data_dir.sh data/train

  touch data/train/.vtln.done
...
if [ ! -f exp/tri4/.done ]; then
  steps/align_si.sh \
    --boost-silence $boost_sil --nj $train_nj --cmd "$train_cmd" \
    data/train_novtln data/lang exp/tri3 exp/tri3_ali
